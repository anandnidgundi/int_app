...
class BM_TasksModel extends Model
{
    ...
    public function getBM_TaskListForAdmin($role, $user, $selectedMonth, $selectedBranch, $selectedCluster, $selectedZone, $selectedDate, $selectedToDate)
    {
      
        $db2 = \Config\Database::connect('secondary'); // For emp and branches data
 
        // Get employee and branch data upfront
        ...
        log_message('error', 'Selected Date :  ' . print_r($selectedDate, true));
 
        // Main query builder
        $builder = $this->db->table('bm_tasks m')
            ->select('m.*, m.taskDate as mtaskDate, um.cluster, um.zone')
            ->join('user_map um', 'FIND_IN_SET(m.branch, um.branches)', 'left')
            ->where('DATE_FORMAT(m.taskDate, "%Y-%m")', $selectedMonth);
 
        // Add zone filter if provided
        if (!empty($selectedZone) && $selectedZone != '0') {
            $builder->where('um.zone', $selectedZone);
        }else if (!empty($selectedCluster) && $selectedCluster != '0') {
            $builder->where('um.cluster', $selectedCluster);
        }else  if (!empty($selectedBranch) && $selectedBranch != '0') {
    ...
    $builder->where('m.taskDate >=', $selectedDate);
    $builder->where('m.taskDate <=', $selectedToDate);
    ...
    public function getBM_TaskListForAdminforbranch($selectedBranch, $selectedDate = null)
{
    $db2 = \Config\Database::connect('secondary'); // db2: emp and branches

    // Load branch and employee data from db2
    $branchData = $db2->table('Branches')->select('id, SysField')->get()->getResultArray();
    $empData = $db2->table('new_emp_master')->select('emp_code, fname, lname')->get()->getResultArray();
    ...
    if ($selectedBranch > 0) {
        ...
        // If no branch selected and no specific date, default to last 3 days
        if (empty($selectedDate)) {
            $today = date('Y-m-d');
            $yesterday = date('Y-m-d', strtotime('-1 day'));
            $dayBeforeYesterday = date('Y-m-d', strtotime('-2 days'));

            $builder->whereIn('m.taskDate', [$today, $yesterday, $dayBeforeYesterday]);
        }
    }

    // If selectedDate is passed and not empty, use it as filter
    if (!empty($selectedDate)) {
        $builder->where('m.taskDate', $selectedDate);
    }
    ...
    // Enhance results with emp and branch info
    foreach ($results as &$task) {
        $taskId = $task['mid'];
        ...
        // Optional: Fetch related task details (can be commented out if not needed)
        $relatedTables = [
            'report_amendments' => 'report_amendments',
            'repeat_punctures' => 'repeat_punctures',
            'repeat_samples' => 'repeat_samples',
    ...
    // Remove duplicate tasks based on mid
    $results = array_values(array_reduce($results, function ($carry, $item) {
        $carry[$item['mid']] = $item;
        return $carry;
    }, []));
    ...
    public function getBM_TaskListForAdmin_backup($role, $user, $selectedMonth, $selectedBranch, $selectedCluster)
    {
