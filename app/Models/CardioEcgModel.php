...
class CardioEcgModel extends Model
{
    protected $table = 'cardiologist_ecg'; // Table name
    protected $primaryKey = 'id'; // Primary key

    // Fields that can be inserted/updated
    protected $allowedFields = ['doctor_name', 'nid', 'doctor_count', 'bmid'];
 
    public function getCardioEcgData($nid){
    ...
    protected $validationMessages = [
        ...
        public function saveCardioEcgData()
    {
        $CardioEcgModel = new \App\Models\CardioEcgModel();
        ...
        if ($CardioEcgModel->save($data)) {
            return $this->respond(['status' => true, 'message' => 'Data saved successfully.']);
        } else {
            return $this->respond(['status' => false, 'message' => $CardioEcgModel->errors()]);
        }
    }
