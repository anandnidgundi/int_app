...
public function getClustersWithBranches($user) {
    $db2 = \Config\Database::connect('secondary'); // For branches
    $primaryDb = \Config\Database::connect();      // For clusters and user_map
    ...
    // Step 3: Fetch allowed cluster IDs for the user from user_map
    $userMap = $primaryDb->table('user_map')
        ->select('cluster')
        ->where('emp_code', $user)
        ->get()
        ->getRow();

    // Handle case where no mapping exists
    if (!$userMap || empty($userMap->cluster)) {
        return []; // Or return an error response
    }

    // Parse comma-separated cluster IDs
    $allowedClusterIds = array_map('trim', explode(',', $userMap->cluster));

    // Step 4: Fetch only allowed clusters from clusters table
    $clusterList = $primaryDb->table('clusters')
        ->select('*')
        ->where('status', 'A')
        ->whereIn('cluster_id', $allowedClusterIds)
        ->get()
        ->getResultArray();

    // Step 5: Attach branch names to each cluster
    foreach ($clusterList as &$cluster) {
        $branchNames = [];
            ...
            //    return $this->db->table( 'clusters' )
                ...
                public function assignZoneToEmployee($data)
                {
                    ...
                    if ($existing) {
                        ...
                        public function getUserMap_backup($id){
                            return $this->db->table( 'user_map' )
                            ->select( '*' )
                            ->where( 'emp_code' ,$id)
                            // ->where( 'status', 'A')
                        ...
                        public function getUserMap($id) {
                            $primaryDb = \Config\Database::connect(); // Main DB
                            $db2 = \Config\Database::connect('secondary'); // Secondary DB
                        
                            // Step 1: Get user mapping
                            $userMaps = $primaryDb->table('user_map')
                                ->select('*')
                                ->where('emp_code', $id)
                                ->get()
                                ->getResultArray();
                            ...
                            // Step 2: Prepare mapping data
                            $zoneList = $primaryDb->table('zones')
                                ->select('z_id, zone')
                                ->where('status', 'A')
                                ->get()
                                ->getResultArray();
                            ...
                            foreach ($clusterList as $c) {
                                $clusterMap[$c['cluster_id']] = $c['cluster'];
                            }
                            ...
                            foreach ($branchList as $b) {
                                $branchMap[$b['id']] = $b['SysField'];
                            }
                        
                            // Step 3: Add names to each user map entry
                            foreach ($userMaps as &$userMap) {
                                // Zone names
                                $zoneIds = explode(',', $userMap['zone']);
                                $zoneNames = [];
                                foreach ($zoneIds as $zid) {
                                    $zid = trim($zid);
                                    if (isset($zoneMap[$zid])) {
                                        $zoneNames[] = $zoneMap[$zid];
                                    }
                                }
                                $userMap['zone_names'] = implode(', ', $zoneNames);
                        
                                // Cluster names
                                $clusterIds = explode(',', $userMap['cluster']);
                                $clusterNames = [];
                                foreach ($clusterIds as $cid) {
                                    $cid = trim($cid);
                                    if (isset($clusterMap[$cid])) {
                                        $clusterNames[] = $clusterMap[$cid];
                                    }
                                }
                                $userMap['cluster_names'] = implode(', ', $clusterNames);
                        
                                // Branch names
                                $branchIds = explode(',', $userMap['branches']);
                                $branchNames = [];
                                foreach ($branchIds as $bid) {
                                    $bid = trim($bid);
                                    if (isset($branchMap[$bid])) {
                                        $branchNames[] = $branchMap[$bid];
                                    }
                                }
                                $userMap['branch_names'] = implode(', ', $branchNames);
                            }
                        ...
                        public function avpdashboardCount()
{
    $startDate = date('Y-m-d', strtotime('-7 days'));
    $endDate = date('Y-m-d'); // today

    $bmTasks = $this->db->table('bm_tasks')
        ->selectCount('*', 'bmTasksCount')
        ->where('DATE(createdDTM) >=', $startDate)
        ->where('DATE(createdDTM) <=', $endDate)
        ->get()
        ->getRowArray();

    $powerTasks = $this->db->table('power_consumption')
        ->selectCount('*', 'powerConsumptionCount')
        ->where('DATE(createdDTM) >=', $startDate)
        ->where('DATE(createdDTM) <=', $endDate)
        ->get()
        ->getRowArray();

    $dieselTasks = $this->db->table('diesel_consumption')
        ->selectCount('*', 'dieselConsumptionCount')
        ->where('DATE(createdDTM) >=', $startDate)
        ->where('DATE(createdDTM) <=', $endDate)
        ->get()
        ->getRowArray();
    ...
    return [
        'STATUS' => true,
        'data' => array_merge($bmTasks, $powerTasks, $dieselTasks)
    ];
}


public function getMatchedBranches($clusterId)
{
    // 1) Fetch the comma‑list of branch IDs for this cluster
    $cluster = $this->db->table('clusters')
        ->select('branches')
        ->where('cluster_id', $clusterId)
        ->get()
        ->getRowArray();

    if (empty($cluster) || empty($cluster['branches'])) {
        return []; // no branches found
    }

    // 2) Turn it into a clean array of IDs
    $branchIds = array_filter(array_map('trim', explode(',', $cluster['branches'])));
    if (empty($branchIds)) {
        return []; // nothing to look up
    }

    // 3) Query only those IDs from the secondary DB, selecting both id and SysField
    $db2 = \Config\Database::connect('secondary');
    $branchRows = $db2->table('Branches')
        ->select('id, SysField')
        ->whereIn('id', $branchIds)
        ->where('status', 'A')
        ->get()
    ...
    // 4) Return each row as ['id' => ..., 'SysField' => ...]
    return $branchRows;
}



public function getYesterdaysReading($user)
{
    return $this->db->table('power_consumption')
        ->select('nonbusinesshours')
        ->where('createdBy', $user)
        ->orderBy('createdDTM', 'DESC')
        ->limit(1)
        ->get()
        ->getRowArray(); // get only one row
}
