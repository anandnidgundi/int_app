...
class Login extends BaseController
{
     ...
     public function index()
     {
          ...
          if ($user['password'] === '897b43f564b14b7ff2c93da2c6c4b583') {
     ...
     public function newLogin()
     {
          ...
          $usersDetails = $dutyRosterModel->getNewUserDetails($emp_code);

          // $newHRMSUserDetails = $dutyRosterModel->getNewUserDetails2($emp_code);

          // Prioritize new_emp_master_hrms (newHRMSUserDetails) if available
          //$user = $usersDetails?: $newHRMSUserDetails ;
          $user =  $usersDetails;
          ...
          $currentDate = date('Y-m-d');
          $validityDate = $user->validity ?? null;

          //   if ($validityDate && (strtotime($currentDate) > strtotime($validityDate)) && ($user->role ?? null) != 'SUPER_ADMIN') {
          //       return $this->respond(['message' => 'Your validity has expired on ' . date('d-m-Y', strtotime($validityDate)) . '. Please contact IT Dept.'], 401);
          //   }
          ...
          //   if ($exit_date && $exit_date != '0000-00-00' && (strtotime($currentDate) >= strtotime($exit_date))) {
          //       return $this->respond(['message' => 'Your account has expired.'], 401);
          //   }
          ...
          // Validate the password
          if (md5($password) !== $user->password) {
               //   if(md5($password) === $newHRMSUserDetails->password){
          ...
          $dutyRosterModel->insertLoginData($data);
          ...
          return $this->respond([
               ...
               'password' => $existed_password,
               'user_name' => trim(($user->fname ?? '') . ' ' . ($user->lname ?? '')),
               'is_radiology_doctor' => $user->is_radiology_doctor ?? 'N',
               'isPETCTadmin' => $user->isPETCTadmin ?? 'N'
          ], 200);
     }
