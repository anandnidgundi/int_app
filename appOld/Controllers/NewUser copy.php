...
class NewUser extends BaseController
{
     ...
     public function updateEmployee($empId = null)
     {
          try {
               ...
               log_message('debug', 'Request payload: ' . json_encode($employeeData));
     ...
     public function deleteEmployee($empCode = null)
     {
          try {
               ...
               // Ensure emp_id exists in the employee record
               $empId = $employee['emp_id'] ?? $employee['id'] ?? null;
               if (!$empId) {
                    return $this->respond([
                         'status' => false,
               ...
               $result = $newUserModel->update($empId, $updateData);
