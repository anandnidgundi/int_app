...
class BM_Tasks extends BaseController
{
    ...
    public function getBM_TaskListForAdmin()
    {
        ...
        // Set selected month from request, default to current month if not provided
        $selectedMonth = $requestData->month ?? date('Y-m');
 
        // Set selected branch(es)
        $selectedBranch = [];
        if (!empty($requestData->branch_id) && $requestData->branch_id != '0') {
            $selectedBranch = [$requestData->branch_id];
        }
 
        // Set selected cluster and zone from request
        $selectedCluster = (!empty($requestData->cluster_id))
            ? $requestData->cluster_id
            : '0';
           
        $selectedZone = (!empty($requestData->zone_id))
            ? $requestData->zone_id
            : '0';
         
        $selectedDate = (!empty($requestData->selectedDate))
            ? $requestData->selectedDate
            : '';

        $selectedToDate = (!empty($requestData->selectedToDate))
        ? $requestData->selectedToDate
        : '';    
 
        // Pass the parameters to the model
        $mtDetails = $bmTasksModel->getBM_TaskListForAdmin($role, $user, $selectedMonth, $selectedBranch, $selectedCluster, $selectedZone, $selectedDate, $selectedToDate);
    ...
    public function getBM_TaskListForAdminforbranch()
    {
        // First validate authorization
        $userDetails = $this->validateAuthorization();
        if (!$userDetails) {
        ...
        // Pass the parameters to the model
        $mtDetails = $bmTasksModel->getBM_TaskListForAdminforbranch($requestData->branch_id);
    ...
    public function getBM_TaskListForAdmin_backup()
    {
        // First validate authorization
        $userDetails = $this->validateAuthorization();
        if (!$userDetails) {
        ...
        // Pass the parameters to the model
        $mtDetails = $bmTasksModel->getBM_TaskListForAdmin($role, $user, $selectedMonth, $selectedBranch, $selectedCluster, $selectedZone);
