...
class User extends BaseController
{
    ...
    public function index()
    {
     ...
     public function getShiftRoaster()
     {
        ...
        //  $user = $userDetails['user_code'];
          ...
          $userModel = new UserModel();
          $user = $userModel->getShiftRoaster();

          if ($user) {
               return $this->respond(['status' => true, 'data' => $user], 200);
          } else {
               return $this->respond(['status' => false, 'message' => 'Shift Roaster not found'], 404);
          }
     }


     public function createShiftRoaster()
     {
          // $userDetails = $this->validateAuthorization();
          // $user = $userDetails->emp_code;
          $request = $this->request->getJSON(true); // Get as associative array
          $shift_roaster = $request['roster'] ?? [];

          $data = [
               "shift_roaster" => $shift_roaster,
               'created_by'   => $user ?? '110104',
               'created_on'   => Time::now('Asia/Kolkata', 'en_US')->toDateTimeString(),
               'modified_on'   => Time::now('Asia/Kolkata', 'en_US')->toDateTimeString()
          ];
          ...
          $user = $userModel->createShiftRoaster($data);
          if ($user) {  // true on success
               return $this->respond(['status' => true, 'data' => $user], 200);
          } else {
     ...
     public function getShiftRoasterById($id)
     {
          $userModel = new UserModel();
          $user = $userModel->getShiftRoasterById($id);
          if ($user) {
               return $this->respond(['status' => true, 'data' => $user], 200);
          } else {
               return $this->respond(['status' => false, 'message' => 'Shift Roaster not Found'], 500);
          }
     }



     public function updateShiftRoasterById($id)
     {
          $userModel = new UserModel();
          $request = $this->request->getJSON(true); // Get as associative array
          $shift = $request['roster'] ?? [];
          $data = [
               'shift' => $shift,
               'modified_by' => $user ?? '110104',
               'modified_on' => Time::now('Asia/Kolkata', 'en_US')->toDateTimeString()
          ];
          $user = $userModel->updateShiftRoasterById($id, $data);
          if ($user) {
               return $this->respond(['status' => true, 'data' => $user], 200);
          } else {
               return $this->respond(['status' => false, 'message' => 'Shift Roaster not Updated'], 500);
          }
     }

     public function deleteShiftRoasterById($id)
     {
          $userModel = new UserModel();
          $data = [
               'status' => 'I',
          ...
          $user = $userModel->deleteShiftRoasterById($id, $data);
          if ($user) {
               return $this->respond(['status' => true, 'message' => 'Shift Roaster Deleted Successfully'], 200);
          } else {
               return $this->respond(['status' => false, 'message' => 'Shift Roaster not Deleted'], 500);
          }
     }
