...
class UsgModel extends Model
{
    protected $table = 'usg'; // Table name
    ...
    public function getUsgData($nid){
        $data = $this->where('nid', $nid)->findAll();
        return $data;
    }

        public function saveUsgData()
    {
        $UsgModel = new \App\Models\UsgModel();
        ...
        if ($UsgModel->save($data)) {
            return $this->respond(['status' => true, 'message' => 'Data saved successfully.']);
        } else {
            return $this->respond(['status' => false, 'message' => $UsgModel->errors()]);
        }
    }
